...
export async function startClient(clientId, options = {}) {
  ...
  client.on('message', async (message) => {
    // 1. Ambil URL Webhook dari .env
            const webhookUrl = process.env.ROM_WEBHOOK_URL;
            if (!webhookUrl) {
                console.warn('ROM_WEBHOOK_URL tidak diatur di .env, webhook dibatalkan.');
                return;
            }
            ...
            try {
                // Kita gunakan fetch (bawaan Node.js v18+)
                ...
                if (response.ok) {
                    // console.log(`Webhook berhasil dikirim ke ROMS untuk ${message.from}`);
                } else {
                    console.error(`Webhook GAGAL dikirim ke ROMS. Status: ${response.status}`);
                }
                
            } catch (error) {
                console.error(`Error saat mengirim webhook ke ROMS: ${error.message}`);
            }
    try {
      const acc = await repo.findOne({ where: { clientId } });
      if (acc) {
